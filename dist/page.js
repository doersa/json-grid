(() => {
  ...
  function activateHover(view2, pos, side, options = {}) {
    var _a2;
    let plugins = view2.state.facet(hoverPlugin).map((p) => view2.plugin(p)).filter((p) => !!p);
    ...
    for (let plugin of plugins)
      plugin.activateHover(view2, pos, side, (_a2 = options.until) !== null && _a2 !== void 0 ? _a2 : (() => false));
  }
  ...
  var searchHighlighter = /* @__PURE__ */ ViewPlugin.fromClass(class {
    ...
    highlight({ query, panel }) {
      ...
      let { view: view2 } = this;
      let builder = new RangeSetBuilder();
      for (let i2 = 0, ranges = view2.visibleRanges, l = ranges.length; i2 < l; i2++) {
  ...
  function applyCompletion(view2, option) {
    ...
    if (typeof apply == "string")
      view2.dispatch(__spreadProps(__spreadValues({}, insertCompletionText(view2.state, apply, result.from, result.to)), {
  ...
  var nextDiagnostic = (view2) => {
    ...
    activateHover(view2, next.from, 1, {
      tooltip: lintHover,
  ...
  var LintGutterMarker = class extends GutterMarker {
    ...
    toDOM(view2) {
      ...
      if (diagnostics.length)
        elt.onmouseover = () => gutterMarkerMouseOver(view2, elt, diagnostics);
      return elt;
    }
  };
  ...
  var lintGutterExtension = /* @__PURE__ */ gutter({
    ...
    widgetMarker: (view2, widget, block) => {
      let diagnostics = [];
      view2.state.field(lintGutterMarkers).between(block.from, block.to, (from, to, value) => {
  ...
  var jsonParseLinter = () => (view2) => {
    try {
      JSON.parse(view2.state.doc.toString());
    } catch (e) {
      if (!(e instanceof SyntaxError))
        throw e;
      const pos = getErrorPosition(e, view2.state.doc);
      return [{
        from: pos,
        message: e.message,
        severity: "error",
    ...
    view.dom.addEventListener("click", () => {
      setTimeout(() => {
        ...
        if (typeof window.__focusPreviewPath === "function") {
          ...
          if (status) status.textContent = "\u53F3\u4FA7\u5B9A\u4F4D \xB7 " + path;
        }
      }, 0);
    });
  }
  ...
  function setEditorSelection(from, to) {
    if (!view) return;
    view.dispatch({ selection: { anchor: from, head: to == null ? from : to } });
    view.focus();
  }
  function revealEditorPosition(offset) {
    if (!view) return;
  ...
  function sourcePathFromDisplayPath(displayPath, root) {
    const base2 = displayBaseKey(root);
    if (!base2 || !displayPath || displayPath === "$") return base2 ? "$." + base2 : displayPath || "$";
    return "$." + base2 + displayPath.slice(1);
  }
  function displayPathFromSourcePath(sourcePath, root) {
    const base2 = displayBaseKey(root);
    const prefix = "$." + base2;
    if (!base2 || !sourcePath) return sourcePath || "$";
  ...
  function resolveParsedStringCursorPath(root, formatted, item, cursor) {
    ...
    let rawIndex = 0;
    let i2 = literalStart + 1;
    const literalEnd = item.valueEnd;
    while (i2 < cursor && i2 < literalEnd) {
      if (formatted[i2] === "\\") i2 += 2;
      else i2 += 1;
      rawIndex += 1;
    }
  ...
  function findStringEnd(text, start) {
    let escaped = false;
    for (let i2 = start + 1; i2 < text.length; i2 += 1) {
      const ch = text[i2];
      if (escaped) {
        escaped = false;
        continue;
  ...
  function findNextSiblingStart(text, start) {
    ...
    for (let i2 = start; i2 < text.length; i2 += 1) {
      ...
      else if (ch === "," && depth === 0) return firstNonSpace(text, i2 + 1);
    }
    return -1;
  }
  ...
  function capturePreviewStateFromDom() {
    try {
      const doc2 = el.preview.contentDocument;
      if (!doc2) return ui.previewState;
      const pathSelect = doc2.getElementById("pathSelect");
      const globalSearch = doc2.getElementById("globalSearch");
      const content2 = doc2.getElementById("content");
      return {
        path: pathSelect ? pathSelect.value : "",
        search: globalSearch ? globalSearch.value : "",
        scrollTop: content2 ? content2.scrollTop : 0,
        scrollLeft: content2 ? content2.scrollLeft : 0,
        openPaths: Array.from(doc2.querySelectorAll("details[data-path][open]")).map((item) => item.getAttribute("data-path"))
      };
    } catch (err) {
  ...
  function scheduleRender() {
    ...
    renderTimer = setTimeout(renderNow, 260);
  }
  function renderNow(options = {}) {
    ...
    const responseText = getEditorValue().trim();
    ...
    let parsedRoot;
    try {
      parsedRoot = JSON.parse(responseText);
    } catch (err) {
    ...
    renderPreview(parsedRoot, responseText);
  }
  function renderPreview(parsedRoot, responseText) {
  ...
  document.getElementById("minifyBtn").onclick = () => {
    try {
      setEditorValue(JSON.stringify(JSON.parse(getEditorValue())));
      renderNow();
    } catch (err) {
  ...
  document.getElementById("foldExpandAll").onclick = () => expandAll();
  document.getElementById("foldCollapseAll").onclick = () => collapseAll();
  initInputPaneToggle();
  initSplitWidth();
  bindSplitResizer();
  initEditor();
  window.__scheduleJsonRender = scheduleRender;
  window.__pathFromEditorCursor = pathFromInputCursor;
  window.__focusPreviewPath = focusPreviewPath;
  window.__getEditorValue = getEditorValue;
  setEditorValue(localStorage.getItem(STORAGE_KEY) || sampleJson);
  renderNow();
})();
