// 行为等价测试：重构后的模板与 legacy 模板逐 DOM 比对。
//
// legacy-template.txt 最初在重构时从单文件版（2623857）捕获，作为独立实现
// 参照：新模块构建须与它 <style> 逐字节一致、渲染输出逐 DOM 相等，从而守住
// “两套实现行为一致”的差分性质。当有意变更表头渲染或样式时（如列冻结按钮
// 扩展到根表），须把等价改动对称地同步进 legacy-template.txt 的 <style> 与
// 内联 <script>，保持差分有效——不要用新构建整体覆盖它，那会让比对退化为
// “新 vs 新”的同义反复。
//
